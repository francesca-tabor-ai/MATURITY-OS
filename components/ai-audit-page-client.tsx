...
import { AIMaturityDashboard } from '@/components/ai-maturity-dashboard';

export function AIAuditPageClient({ organisationId }: { organisationId: string }) {
  const [view, setView] = useState<'form' | 'result' | 'history'>('form');
  const [lastResult, setLastResult] = useState<Record<string, unknown> | null>(null);
  const [history, setHistory] = useState<Array<{ id: string; maturity_stage: number; maturity_score: number; created_at: string }>>([]);
  const [selectedResult, setSelectedResult] = useState<Record<string, unknown> | null>(null);

  useEffect(() => {
    fetch(`/api/organisations/${organisationId}/ai-audit`)
      .then((r) => r.json())
      .then((data) => {
    ...
    setHistory((prev) => [
      {
        ...
        maturity_score: result.maturity_score as number,
  ...
  if (view === 'result' && resultToShow) {
    return (
      <div className="space-y-6">
        <AIMaturityDashboard result={resultToShow as never} />
            ...
            <tbody>
              {history.map((r) => (
                <tr key={r.id} className="border-b border-zinc-100 last:border-0">
                  ...
                  <td className="px-4 py-3">{r.maturity_score}</td>
                  <td className="px-4 py-3">
                    <button
                      type="button"
                      onClick={() => {
                        fetch(`/api/organisations/${organisationId}/ai-audit/${r.id}`)
                          .then((res) => res.json())
                          .then((data) => {
  ...
  if (view === 'history') {
    return (
      <div className="space-y-6">
        <p className="text-zinc-600">No AI audit results yet. Run your first audit below.</p>
  ...
  return (
    <div className="space-y-6">
      <AIAuditForm organisationId={organisationId} onSuccess={handleSuccess} />
      {history.length > 0 && (
